...
/**
  ...
  <tr>
    ...
    <td>Error code generated by the primary. If the primary fails, the
    replica will fail with the same error code.
    </td>
  </tr>
  ...
  <tr>
    ...
    flags except @c MODE_NO_DIR_IN_CREATE are honored by the replica;
    the replica always preserves its old value of @c
  ...
  <tr>
    ...
    that the primary converts the query to when it receives it; this is
  ...
  <tr>
    ...
    <td>Variable-length string: the length in bytes (1 byte) followed
    by the characters (at most 255 bytes).
    <td>The time_zone of the primary.
  ...
  <tr>
    ...
    statement and used by replica to apply filter rules without opening
    all the tables on replica. This is required because some tables may
    not exist on replica because of the filter rules.
    </td>
  </tr>
  <tr>
    <td>primary_data_written</td>
    <td>Q_PRIMARY_DATA_WRITTEN_CODE == 10</td>
    <td>4 byte bitfield</td>

    <td>The value of the original length of a Query_event that comes from a
    primary. Primary's event is relay-logged with storing the original size of
    event in this field by the IO thread. The size is to be restored by reading
    Q_PRIMARY_DATA_WRITTEN_CODE-marked event from the relay log.

    This field is not written to replica's server binlog by the SQL thread.
    This field only exists in relay logs where primary has binlog_version<4 i.e.
    server_version < 5.0 and the replica has binlog_version=4.
    </td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td>The total number and the names to of the databases accessed is stored,
        to be propagated to the replica in order to facilitate the parallel
  ...
  * The status variable Q_CATALOG_CODE == 2 existed in MySQL 5.0.x,
  ...
  Q_CATALOG_CODE will never be written by a new primary, but can still
  be understood by a new replica.
...
public:
  ...
  enum Query_event_status_vars
  {
    ...
    */
    ...
    /*
      ...
      Q_CATALOG_CODE is that then a 5.0.3 replica of this 5.0.x (x>=4)
      primary would crash (segfault etc) because it would expect a 0 when there
      is none.
    */
    Q_CATALOG_NZ_CODE,
    ...
    /*
      ...
      replica in order to facilitate the parallel applying of the Query events.
    */
    Q_UPDATED_DB_NAMES,
    Q_MICROSECONDS,
    /*
   ...
   */
    Q_COMMIT_TS2,
    /*
      The primary connection @@session.explicit_defaults_for_timestamp which
      is recorded for queries, CREATE and ALTER table that is defined with
      a TIMESTAMP column, that are dependent on that feature.
      For pre-WL6292 primary's the associated with this code value is zero.
    */
...
public:
  ...
  */
  ...
  /*
    ...
    primary, we don't know flags2, so use the replica server's global options) and
    flags2==0 (5.0 primary, we know this has a meaning of flags all down which
  ...
  */
  ...
  /*
    ...
    primary of version < 5.0 (i.e., binlog_version < 4). When the IO
    thread writes the relay log, it augments the Query_event with a
    Q_PRIMARY_DATA_WRITTEN_CODE status_var that holds the original event
    ...
    Q_PRIMARY_DATA_WRITTEN_CODE to the replica's server binlog.
  */
  size_t primary_data_written;
  /*
