...
ReplSemiSyncPrimary repl_semisync;
...
static ulong rpl_semi_sync_primary_wait_point= WAIT_AFTER_COMMIT;
				   ...
				   my_off_t log_pos)
{
  ...
  if (repl_semisync.getPrimaryEnabled())
  {
    /*
...
int repl_semi_report_binlog_sync(Binlog_storage_param *param,
                                 ...
                                 my_off_t log_pos)
{
  if (rpl_semi_sync_primary_wait_point == WAIT_AFTER_SYNC)
...
int repl_semi_report_commit(Trans_param *param)
{
  ...
  if (rpl_semi_sync_primary_wait_point == WAIT_AFTER_COMMIT &&
				 ...
				 my_off_t log_pos)
{
  ...
  /*
    semi_sync_replica will be 0 if the user variable doesn't exist. Otherwise, it
    will be set to the value of the user variable.
    'rpl_semi_sync_replica = 0' means that it is not a semisync replica.
  */
  get_user_var_int("rpl_semi_sync_replica", &semi_sync_replica, NULL);

  if (semi_sync_replica != 0)
  {
    if (ack_receiver.add_replica(current_thd))
    {
      sql_print_error("Failed to register replica to semi-sync ACK receiver "
                      "thread.");
      return -1;
    }
  ...
  sql_print_information("Start %s binlog_dump to replica (server_id: %d), pos(%s, %lu)",
...
int repl_semi_binlog_dump_end(Binlog_transmit_param *param)
{
  ...
  sql_print_information("Stop %s binlog_dump to replica (server_id: %d)",
                        semi_sync_replica ? "semi-sync" : "asynchronous",
                        param->server_id);
...
int repl_semi_reset_primary(Binlog_transmit_param *param)
{
  if (repl_semisync.resetPrimary())
    return 1;
  return 0;
}
 ...
 */
static void fix_rpl_semi_sync_primary_timeout(MYSQL_THD thd,
				      SYS_VAR *var,
				      void *ptr,
				      const void *val);

static void fix_rpl_semi_sync_primary_trace_level(MYSQL_THD thd,
					  SYS_VAR *var,
					  void *ptr,
					  const void *val);

static void fix_rpl_semi_sync_primary_wait_no_replica(MYSQL_THD thd,
				      SYS_VAR *var,
				      void *ptr,
				      const void *val);

static void fix_rpl_semi_sync_primary_enabled(MYSQL_THD thd,
				      SYS_VAR *var,
				      void *ptr,
				      const void *val);

static void fix_rpl_semi_sync_primary_wait_for_replica_count(MYSQL_THD thd,
                                                          SYS_VAR *var,
                                                          void *ptr,
                                                          const void *val);

static MYSQL_SYSVAR_BOOL(enabled, rpl_semi_sync_primary_enabled,
  PLUGIN_VAR_OPCMDARG,
 "Enable semi-synchronous replication primary (disabled by default). ",
  NULL, 			// check
  &fix_rpl_semi_sync_primary_enabled,	// update
  0);

static MYSQL_SYSVAR_ULONG(timeout, rpl_semi_sync_primary_timeout,
  PLUGIN_VAR_OPCMDARG,
 "The timeout value (in ms) for semi-synchronous replication in the primary",
  NULL, 			// check
  fix_rpl_semi_sync_primary_timeout,	// update
  10000, 0, ~0UL, 1);

static MYSQL_SYSVAR_BOOL(wait_no_replica, rpl_semi_sync_primary_wait_no_replica,
  PLUGIN_VAR_OPCMDARG,
 "Wait until timeout when no semi-synchronous replication replica available (enabled by default). ",
  NULL, 			// check
  &fix_rpl_semi_sync_primary_wait_no_replica,  // update
  1);

static MYSQL_SYSVAR_ULONG(trace_level, rpl_semi_sync_primary_trace_level,
  PLUGIN_VAR_OPCMDARG,
 "The tracing level for semi-sync replication.",
  NULL,				  // check
  &fix_rpl_semi_sync_primary_trace_level, // update
  32, 0, ~0UL, 1);
...
static MYSQL_SYSVAR_ENUM(
  wait_point,                      /* name     */
  rpl_semi_sync_primary_wait_point, /* var      */
  PLUGIN_VAR_OPCMDARG,             /* flags    */
  "Semisync can wait for replica ACKs at one of two points,"
  ...
  "replica. AFTER_COMMIT means that semisynchronous replication waits just "
...
static MYSQL_SYSVAR_UINT(wait_for_replica_count,   /* name  */
  rpl_semi_sync_primary_wait_for_replica_count,     /* var   */
  PLUGIN_VAR_OPCMDARG,                           /* flags */
  "How many replicas the events should be replicated to. Semisynchronous "
  "replication primary will wait until all events of the transaction are "
  "replicated to at least rpl_semi_sync_primary_wait_for_replica_count replicas",
  NULL,                                           /* check() */
  &fix_rpl_semi_sync_primary_wait_for_replica_count, /* update */
  1, 1, 65535, 1);

static SYS_VAR* semi_sync_primary_system_vars[]= {
				      ...
				      const void *val)
{
  *(unsigned long *)ptr= *(unsigned long *)val;
  repl_semisync.setWaitTimeout(rpl_semi_sync_primary_timeout);
  return;
}
...
{
  *(unsigned long *)ptr= *(unsigned long *)val;
  repl_semisync.setTraceLevel(rpl_semi_sync_primary_trace_level);
  ack_receiver.setTraceLevel(rpl_semi_sync_primary_trace_level);
  return;
}
				      ...
				      const void *val)
{
  *(char *)ptr= *(char *)val;
  ...
  {
    if (repl_semisync.enablePrimary() != 0)
      rpl_semi_sync_primary_enabled = false;
    else if (ack_receiver.start())
    {
      repl_semisync.disablePrimary();
      rpl_semi_sync_primary_enabled = false;
    }
  }
  else
  {
    if (repl_semisync.disablePrimary() != 0)
      rpl_semi_sync_primary_enabled = true;
    ack_receiver.stop();
  }
...
static void fix_rpl_semi_sync_primary_wait_for_replica_count(MYSQL_THD thd,
                                                          ...
                                                          const void *val)
{
  (void) repl_semisync.setWaitReplicaCount(*(unsigned int*) val);
  return;
}
...
static void fix_rpl_semi_sync_primary_wait_no_replica(MYSQL_THD thd,
				      ...
				      const void *val)
{
  if (rpl_semi_sync_primary_wait_no_replica != *(char *)val)
  {
    *(char *)ptr= *(char *)val;
...
Binlog_transmit_observer transmit_observer = {
  ...
  repl_semi_reset_primary,	// reset
};


#define SHOW_FNAME(name)			\
  rpl_semi_sync_primary_show_##name
...
#define DEF_SHOW_FUNC(name, show_type)					\
  ...
  {									\
    ...
    var->type= show_type;						\
    var->value= (char *)&rpl_semi_sync_primary_##name;				\
    return 0;								\
  }
...
static SHOW_VAR semi_sync_primary_status_vars[]= {
  {"Rpl_semi_sync_primary_status",
   (char*) &SHOW_FNAME(status),
   SHOW_FUNC, SHOW_SCOPE_GLOBAL},
  {"Rpl_semi_sync_primary_clients",
   (char*) &SHOW_FNAME(clients),
   SHOW_FUNC, SHOW_SCOPE_GLOBAL},
  {"Rpl_semi_sync_primary_yes_tx",
   (char*) &rpl_semi_sync_primary_yes_transactions,
   SHOW_LONG, SHOW_SCOPE_GLOBAL},
  {"Rpl_semi_sync_primary_no_tx",
   (char*) &rpl_semi_sync_primary_no_transactions,
   SHOW_LONG, SHOW_SCOPE_GLOBAL},
  {"Rpl_semi_sync_primary_wait_sessions",
   (char*) &SHOW_FNAME(wait_sessions),
   SHOW_FUNC, SHOW_SCOPE_GLOBAL},
  {"Rpl_semi_sync_primary_no_times",
   (char*) &rpl_semi_sync_primary_off_times,
   SHOW_LONG, SHOW_SCOPE_GLOBAL},
  {"Rpl_semi_sync_primary_timefunc_failures",
   (char*) &rpl_semi_sync_primary_timefunc_fails,
   SHOW_LONG, SHOW_SCOPE_GLOBAL},
  {"Rpl_semi_sync_primary_wait_pos_backtraverse",
   (char*) &rpl_semi_sync_primary_wait_pos_backtraverse,
   SHOW_LONG, SHOW_SCOPE_GLOBAL},
  {"Rpl_semi_sync_primary_tx_wait_time",
   (char*) &SHOW_FNAME(trx_wait_time),
   SHOW_FUNC, SHOW_SCOPE_GLOBAL},
  {"Rpl_semi_sync_primary_tx_waits",
   (char*) &SHOW_FNAME(trx_wait_num),
   SHOW_FUNC, SHOW_SCOPE_GLOBAL},
  {"Rpl_semi_sync_primary_tx_avg_wait_time",
   (char*) &SHOW_FNAME(avg_trx_wait_time),
   SHOW_FUNC, SHOW_SCOPE_GLOBAL},
  {"Rpl_semi_sync_primary_net_wait_time",
   (char*) &SHOW_FNAME(net_wait_time),
   SHOW_FUNC, SHOW_SCOPE_GLOBAL},
  {"Rpl_semi_sync_primary_net_waits",
   (char*) &SHOW_FNAME(net_wait_num),
   SHOW_FUNC, SHOW_SCOPE_GLOBAL},
  {"Rpl_semi_sync_primary_net_avg_wait_time",
...
{ 0, "Waiting for semi-sync ACK from replica", 0};

PSI_stage_info stage_waiting_for_semi_sync_replica=
{ 0, "Waiting for semi-sync replica connection", 0};

PSI_stage_info stage_reading_semi_sync_ack=
{ 0, "Reading semi-sync ACK from replica", 0};
...
static int semi_sync_primary_plugin_init(void *p)
{
#ifdef HAVE_PSI_INTERFACE
...
static int semi_sync_primary_plugin_deinit(void *p)
{
  ack_receiver.stop();
...
struct Mysql_replication semi_sync_primary_plugin= {
...
*/
mysql_declare_plugin(semi_sync_primary)
{
  MYSQL_REPLICATION_PLUGIN,
  &semi_sync_primary_plugin,
  "rpl_semi_sync_primary",
  "He Zhenxing",
  "Semi-synchronous replication primary",
  PLUGIN_LICENSE_GPL,
  semi_sync_primary_plugin_init, /* Plugin Init */
  semi_sync_primary_plugin_deinit, /* Plugin Deinit */
  0x0100 /* 1.0 */,
  semi_sync_primary_status_vars,	/* status variables */
  semi_sync_primary_system_vars,	/* system variables */
  NULL,                         /* config options */
  0,                            /* flags */
}
