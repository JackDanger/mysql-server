...
class MYSQL_BIN_LOG: public TC_LOG
{
  ...
  /*
    ...
    index file crash safe when primary server restarts.
  */
...
public:
  ...
  /*
    ...
    The initial value comes from the replica's local FD that heads
    the very first Relay-Log file. In the following the value may change
    with each received primary's FD_m.
    Besides to be used in verification events that IO thread receives
    (except the 1st fake Rotate, see @c Primary_info:: checksum_alg_before_fd), 
    the value specifies if/how to compute checksum for replica's local events
    and the first fake Rotate (R_f^1) coming from the primary.
    ...
    `_s'   indicates the event is generated by Replica
    `_m'   - by Primary
...
public:
  ...
  {
    /*
      binlog_end_pos is used only on primary's binlog right now. It is possible
...
public:
  ...
  */
  ...
  bool append_buffer(const char* buf, uint len, Primary_info *mi);
  bool append_event(Log_event* ev, Primary_info *mi);
private:
  bool after_append_to_relay_log(Primary_info *mi);
