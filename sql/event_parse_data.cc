...
/**
  ...
  the primary or set to the server_id of the primary if executing on 
  the replica. If executing on replica, also set status to REPLICASIDE_DISABLED.
...
{
  /* Disable replicated events on replica. */
  if ((thd->system_thread == SYSTEM_THREAD_REPLICA_SQL) ||
      (thd->system_thread == SYSTEM_THREAD_REPLICA_WORKER) ||
      (thd->system_thread == SYSTEM_THREAD_REPLICA_IO))
  {
    DBUG_PRINT("info", ("Invoked object status set to REPLICASIDE_DISABLED."));
