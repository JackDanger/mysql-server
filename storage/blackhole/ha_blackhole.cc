...
#include "sql_class.h"                          // THD, SYSTEM_THREAD_REPLICA_*

static PSI_memory_key bh_key_memory_blackhole_share;

static bool is_replica_applier(THD *thd)
{
  return thd->system_thread == SYSTEM_THREAD_REPLICA_SQL ||
...
int ha_blackhole::update_row(const uchar *old_data, uchar *new_data)
{
  ...
  THD *thd= ha_thd();
  if (is_replica_applier(thd) && thd->query().str == NULL)
    DBUG_RETURN(0);
