...
which the primary thread drops in background. We need this on Unix because in
...
as a background operation, which is taken care of by the primary thread
in srv0srv.cc.
@return error code or DB_SUCCESS */
static
dberr_t
...
funct_exit:
	...
	/* No need to immediately invoke primary thread as there is no work
	generated by intrinsic table operation that needs primary thread
	attention. */
