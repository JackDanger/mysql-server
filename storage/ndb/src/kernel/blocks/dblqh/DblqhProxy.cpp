...
DblqhProxy::execLCP_FRAG_ORD(Signal* signal)
{
  ...
  if (c_lcpRecord.m_state == LcpRecord::L_IDLE)
  {
    ...
    if (c_lcpRecord.m_lcpId == req->lcpId)
    {
      jam();
      /**
       * In primary take over the new primary doesn't get the perfect view
       * of our state immediately. So he can resend LCP_FRAG_ORDs that
       * were already sent. In this case we have already completed the
       * LCP and the new primary is sending messages which it doesn't
  ...
  if (c_lcpRecord.m_lastFragmentFlag)
  {
     ...
     * generated by a new primary that didn't know that the old primary already
...
DblqhProxy::execLOCAL_RECOVERY_COMP_REP(Signal *signal)
{
  ...
  ss.phaseToSend++;
  Uint32 primaryNodeId = refToNode(ss.m_req.senderRef);
  Uint32 primary_version = getNodeInfo(primaryNodeId).m_version;
  if (primary_version >= NDBD_NODE_RECOVERY_STATUS_VERSION)
  {
    /* Only send this information to primarys that have code to handle it. */
    jam();
